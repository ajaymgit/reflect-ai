...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen page-gradient text-white pb-24 md:pb-0">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#101a15cc] backdrop-blur-xl px-4 md:px-6 py-3 md:py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <Link to="/dashboard" className="hover:opacity-90 transition">
            ...
            <h1 className="text-base md:text-[1.35rem] font-semibold">{title}</h1>
            <p className="hidden md:block text-[11px] text-white/55 mt-0.5">Home / {title}</p>
          </Link>
      ...
      {showTip ? (
        <div className="px-4 md:px-6 pt-3">
          <div className="max-w-7xl mx-auto rounded-xl border border-brand-200/40 bg-brand-300/15 px-3 py-2 text-sm text-white/85 flex items-center justify-between gap-3">
            <span>New here? Start with Home -&gt; Journal -&gt; Chat -&gt; Health.</span>
        ...
        <div className="grid grid-cols-5 gap-1 px-2 py-1.5">
          {navItems.map(({ to, label, Icon }) => (
            <Link
              ...
              to={to}
              aria-current={location.pathname === to ? "page" : undefined}
              className={`rounded-lg py-2 px-1.5 text-center text-[11px] ${
                location.pathname === to
